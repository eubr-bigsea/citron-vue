        ...
        <div class="row small-padding">
            ...
            <div class="col-md-12">
                <table class="table table-striped table-bordered">
                    <thead>
                        <tr>
                            ...
                            <th class="sortable primary text-center col-md-1" @click="sort('wokflow_id')">
                                Workflow Id <span class="fa" :class="['fa-' + (this.asc === 'false' ? 'sort-up': 'sort-down')]" v-show="orderBy === 'name'"></span>
                                </th>
                            <th class="sortable primary text-center col-md-3" @click="sort('wokflow_name')">
                    ...
                    <tbody>
                        <tr v-for="job in pageData.data">
                            ...
                            <td>
                                <router-link :to="{name: 'editor', params: {id: job.workflow_id }}">{{job.workflow_name}}</router-link>
                            </td>
                            <td>{{job.user_name}}</td>
                            <td class="text-center">{{ formatDate(job.updated, 'DD-MM-YYYY HH:mm:ss') }}</td>
                        </tr>
                    </tbody>
                </table>
                ...
                <nav>
                    <ul class="pager">
                        <li>
                            <router-link :to="{name: 'job-page', params: {page: page - 1 }}" v-show="pageData.pagination.page !== 1">Previous</router-link>
                        </li>
                        <li>
                            <router-link :to="{name: 'job-page', params: {page: page + 1 }}" v-show="pageData.pagination.page !== pageData.pagination.pages">Next</router-link>
                        </li>
                    </ul>
                </nav>
            </div>
